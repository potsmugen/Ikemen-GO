...
type CharCompiler struct {
	...
	token            string
	playerNo         int
	gi               *CharGlobalInfo // Target CGI for this compile, decoupled from sys.cgi[playerNo] so a
	// background Turns preload compiles into its own staging area instead of racing sys.cgi.
...
func (c *CharCompiler) expValue(out *BytecodeExp, in *string,
	...
	case "command", "selfcommand":
		// Unlike name/gamemode/etc., this always needs a literal to check
		// against a known command - it can't just push a string value.
		opc := OC_command
	...
	case "movetype", "p2movetype", "prevmovetype":
		// The flags here (A/H/I) is a fixed set, not a string
...
func (c *CharCompiler) paramTrans(is IniSection, sc *StateControllerBase, prefix string, id byte) error {
	...
	return c.stateParam(is, prefix+"trans", false, func(data string) error {
		...
		if tt == TT_sub && c.gi.ikemenver[0] == 0 && c.gi.ikemenver[1] == 0 {
			exp[0] = sc.iToExp(defsrc)[0]
			exp[1] = sc.iToExp(defdst)[0]
		}
...
func (c *CharCompiler) charWarn() string {
	...
	return fmt.Sprintf("WARNING: %v's state %v in %v, line %v: ", c.gi.name, c.stateNo, file, c.i+offset)
}
