...
func (ai *AfterImage) recAndCue(sd *SprData, playerNo int, rec bool, hitpause bool, layer int32) {
...
func (e *Explod) update(playerNo int) {
	...
	// Record afterimage
	e.aimg.recAndCue(sd, playerNo, sys.tickNextFrame() && act, sys.tickNextFrame() && e.ignorehitpause && (e.supermovetime != 0 || e.pausemovetime != 0), e.layerno)
...
func (p *Projectile) cueDraw() {
	...
	if p.ani != nil {
		...
		p.aimg.recAndCue(sd, p.playerno, sys.tickNextFrame() && notpause, false, p.layerno)
		sprs.add(sd)
...
func (c *Char) cueDraw() {
	...
	if c.anim != nil {
		...
		// Record afterimage
		c.aimg.recAndCue(charSD, c.playerNo, rec, sys.tickNextFrame() && c.hitPause(), c.layerNo)
		// Hitshake effect
