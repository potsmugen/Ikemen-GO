...
func (ai *AfterImage) recAndCue(sd *SpriteData, playerNo int, rec bool, hitpause bool) {
	...
	// But that is less efficient because then "framegap = 1" afterimages duplicate the current frame of the character
...
func (e *Explod) cueDraw() {
	...
	if e.interpolate {
		e.Interpolate(act, &scale, &alp, &rot, &fLength, &xshear)
	}
