...
func (c *Char) removeExplod(id, idx int32) {
	...
	newSlice := (*playerExplods)[:0] // Reuse backing array
	for _, e := range *playerExplods {
		if e.id != IErr {
			newSlice = append(newSlice, e)
		}
	}
	*playerExplods = newSlice
}
...
// Get the character as well as all its proxy children (and their proxy children)
...
func (cl *CharList) getPlayerWithID(id int32) *Char {
	if id < 0 {
		return nil
	}
