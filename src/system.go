...
func (s *System) SetupCharRoundStart() {
	...
	// What this does is make characters weaker as they accumulate wins in Random Test mode
	for i, p := range s.chars {
	...
	for i, p := range s.chars {
		if len(p) > 0 {
			...
			if sys.autolevel {
				...
				p[0].lifeMax = Max(1, int32(math.Floor(foo*float64(lmax))))
			} else {
