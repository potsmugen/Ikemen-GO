...
func readLifeBar(pre string, is IniSection, sff *Sff, at AnimationTable, f map[int]*Fnt) *LifeBar {
	...
	// Text fields
	lb.value[0] = readLbText(pre+"value.", is, "%d", 0, f, 0)
	for k, v := range readMultipleLbText(pre, "value", is, "%d", 0, f, 0) {
		lb.value[k] = v
	}

	lb.red_value[0] = readLbText(pre+"red.value.", is, "%d", 0, f, 0)
...
func (fs *FightScreen) appendAction(c *Char, msg *LbMsg, s_ffx, a_ffx string, snd, spr [2]int32, anim int32, top bool) {
	if c.teamside < 0 {
		return
	}
