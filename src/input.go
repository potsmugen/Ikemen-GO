...
func ControllerState(kc KeyConfig) [14]bool {
	...
	// For negative button indices (axes as buttons, e.g. triggers), handle like JoystickState
	...
	// For each input index, if its mapping is < 0, replace the value with axis logic
	axisIndices := []int{kc.dU, kc.dD, kc.dL, kc.dR, kc.kA, kc.kB, kc.kC, kc.kX, kc.kY, kc.kZ, kc.kS, kc.kD, kc.kW, kc.kM}
	for i, btnIdx := range axisIndices {
		if btnIdx < 0 {
			out[i] = handleAxisBtn(btnIdx)
		}
	}
