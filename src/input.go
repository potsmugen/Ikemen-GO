...
// Get the state of any symbol/letter combination
...
func (c *Command) GreaterCheckFail(i int, ibuf *InputBuffer) bool {
	...
	// Because diagonals and L/R get in the way when you're not deliberately checking them
	var useLR, useDiag bool
	for _, k := range c.steps[i].keys {
		switch k.key {
		case CK_L, CK_R:
			useLR = true
		case CK_DF, CK_DB, CK_UF, CK_UB:
			useDiag = true
		case CK_DL, CK_DR, CK_UL, CK_UR:
			useLR = true
			useDiag = true
		}
	}

	// Gather all recently changed keys
	changed := make(map[CommandKey]int32)
	addIfRecent := func(ck CommandStepKey) {
		val := ibuf.State(ck)
		if val == 1 || val == -1 {
			changed[ck.key] = val
		}
	...
	if useDiag {
		if useLR {
			addIfRecent(CommandStepKey{key: CK_DL})
			addIfRecent(CommandStepKey{key: CK_DR})
			addIfRecent(CommandStepKey{key: CK_UL})
			addIfRecent(CommandStepKey{key: CK_UR})
		} else {
			addIfRecent(CommandStepKey{key: CK_DF})
			addIfRecent(CommandStepKey{key: CK_DB})
			addIfRecent(CommandStepKey{key: CK_UF})
			addIfRecent(CommandStepKey{key: CK_UB})
		}
	} else if useLR {
		addIfRecent(CommandStepKey{key: CK_L})
		addIfRecent(CommandStepKey{key: CK_R})
	} else {
		addIfRecent(CommandStepKey{key: CK_B})
		addIfRecent(CommandStepKey{key: CK_F})
	}

	addIfRecent(CommandStepKey{key: CK_U})
	addIfRecent(CommandStepKey{key: CK_D})

	// Buttons
	buttons := []CommandKey{CK_a, CK_b, CK_c, CK_x, CK_y, CK_z, CK_s, CK_d, CK_w, CK_m}
	for _, btn := range buttons {
		addIfRecent(CommandStepKey{key: btn})
	}

	// Fail if any recently changed key is not part of the step
	for changedKey := range changed {
		allowed := false
		for _, stepKey := range c.steps[i].keys {
			if changedKey == stepKey.key {
				allowed = true
				break
			}
		}
		if !allowed {
			return true // Invalid change. Fail step
		}
	}
