...
func readClsn(lines []string, i *int, clsnType byte, size int32) ([][4]float32, error) {
	...
	for count < size && *i < len(lines) {
		...
		if boxIdx < 0 || boxIdx >= size || filled[boxIdx] {
			...
			if requested >= 0 {
				LogMessage("WARNING: Clsn[%v] index [%v] collides or is out of range; placed in slot [%v]",
					clsnType, requested, boxIdx)
			}
		}
	...
	if count < size {
		return clsn, Error(fmt.Sprintf(
			"Clsn[%v]: declared %v boxes but only %v found", clsnType, size, count))
	}
