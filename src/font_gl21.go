...
func (f *Font_GL21) Printf(x, y float32, scale float32, spacingXAdd float32,
	...
	// In GL21, this handles the shader switch and enables the vertex pointers
	// since there is no VAO to remember them.
	fr.SetFontPipeline()
	...
	//set text color
	// Updated to use the 64-bit keyed cache (program << 32 | loc)
